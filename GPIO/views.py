...
def fetch_temperatures(request):
    ...
    """
    data = list(
        SensorValues.objects.annotate(measurement=F("temperature")).values(
            "measurement", "timestamp", "is_plausible"
        )
    )
    filter_data = data[-10:]
...
def fetch_vocs(request):
    ...
    data = list(
        SensorValues.objects.annotate(measurement=F("voc")).values(
            "measurement", "timestamp", "is_plausible"
        )
    )
    filter_data = data[-10:]
